...
class MainShell extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // ✅ u est User? — on utilise u?.role avec fallback 'student'
    final role = userAsync.maybeWhen(
      data: (u) => u?.role ?? 'student',
      orElse: () => 'student',
    );

    final hasAdminAccess =
        ['admin', 'university_admin', 'class_leader'].contains(role);
  ...
  IconData _getAdminIcon(String role) {
    switch (role) {
      ...
      case 'university_admin': return Icons.account_balance_outlined;
      case 'class_leader':     return Icons.school_outlined;
  ...
  Color _getAdminColor(String role) {
    switch (role) {
      ...
      case 'university_admin': return Colors.blue;
      case 'class_leader':     return Colors.purple;
  ...
  String _getAdminLabel(String role) {
    switch (role) {
      ...
      case 'university_admin': return 'Univ.';
      case 'class_leader':     return 'Délégué';
