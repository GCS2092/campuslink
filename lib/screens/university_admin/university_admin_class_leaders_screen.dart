...
/// Écran de gestion des responsables de classe pour les administrateurs d'université
class UniversityAdminClassLeadersScreen extends StatefulWidget {
  const UniversityAdminClassLeadersScreen({super.key});

  @override
  State<UniversityAdminClassLeadersScreen> createState() => _UniversityAdminClassLeadersScreenState();
}

class _UniversityAdminClassLeadersScreenState extends State<UniversityAdminClassLeadersScreen> {
  ...
  Future<void> _loadClassLeaders() async {
    ...
    try {
      final leaders = await _universityAdminService.getClassLeaders();
      setState(() {
  ...
  Future<void> _handleAssignClassLeader(String userId) async {
    try {
      final result = await _universityAdminService.assignClassLeader(userId);
      if (mounted) {
  ...
  Future<void> _handleRevokeClassLeader(String userId) async {
    try {
      final result = await _universityAdminService.revokeClassLeader(userId);
      if (mounted) {
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(
