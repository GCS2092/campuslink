...
import { getCampuses } from '../../services/universityAdminService';

export default function UniversityAdminCampusesScreen() {
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Campus' }} />
      <FlatList
        data={list}
        keyExtractor={(x) => String(row(x).id ?? '')}
        contentContainerStyle={styles.list}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); await load(); setRefreshing(false); }} />}
        ListEmptyComponent={<Text style={styles.empty}>Aucun campus</Text>}
        renderItem={({ item: x }) => (
